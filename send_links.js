...
for (var i = 0; i < elementos.length; i++) {
    ...
    var urlRelativa = textoOnclick.match(/recuperaRepresentacionImpresa\(\'[^']+/);    
    //si la encontramos:
    if (urlRelativa.length > 0) {
        //extraemos el ID
        var id=urlRelativa[0].replace(/recuperaRepresentacionImpresa\(\'/g,"");                
        var urlAbsoluta = "https://portalcfdi.facturaelectronica.sat.gob.mx/RepresentacionImpresa.aspx?Datos=" + id;        
